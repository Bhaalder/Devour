...
public class PlayerBaseState : State {
    ...
    private void JumpCheck() {
        ...
        if (owner.IsWallSliding && Input.GetButtonDown("Jump")) {
            Jump(0);
            GameObject particle = Instantiate(owner.JumpParticle, owner.GroundCheck.position - new Vector3(0, 0, 0), Quaternion.identity);
            return;
        }
        if (owner.IsGrounded && Input.GetButtonDown("Jump")) {
            Jump(0);
            GameObject particle = Instantiate(owner.JumpParticle, owner.GroundCheck.position - new Vector3(0, 1.5f, 0), Quaternion.identity, owner.GroundCheck);
            return;
        }
        ...
        if (owner.HasAbility(PlayerAbility.DOUBLEJUMP)) {
            ...
            if (owner.PlayerState == PlayerState.AIR && owner.ExtraJumpsLeft > 0 && Input.GetButtonDown("Jump")) {
                ...
                Jump(0);
                GameObject particle = Instantiate(owner.DoubleJumpParticle, owner.GroundCheck.position - new Vector3(0, 1.5f, 0), Quaternion.identity, owner.GroundCheck);
            }
        }
    }
